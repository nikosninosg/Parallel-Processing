...
int main(int argc, char *argv[])
{
	...
	#pragma omp parallel shared(SimplifiedAllPolylines) private(wtime,polyline, simplified_polyline) firstprivate(epsilon, AllPolylines)
	{
	wtime = omp_get_wtime();	
	...
	cout << "Thread : " << omp_get_thread_num() << ' ' << wtime << "seconds" << "\n";
	}
